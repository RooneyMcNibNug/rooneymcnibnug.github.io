...
Requesting all staff email addresses is one thing - its possible, but what else could we request in terms of emails? Maybe something that would glean more information about what the agency might be up to? What else could we get besides individual staff member email addresses that would be useful, and also that way we can dodge any "hem haw"ing about emails being "sensitive information"?
...
Enter (not so) artisinally-crafted Powershell one-liners within FOIA requests. For example:
